...
contract NotReallyACrossChainToken is ERC20 {
    ...
    // Events
    event RedeemSuccess(address indexed account, uint256 amount);
    ...
    function getXcmMessage(uint128 amount, uint32 paraId, bytes32 beneficiary) external returns (bytes memory) {
        return ICrossChainTransfers(CROSS_CHAIN_TRANSFERS_ADDRESS).teleport(amount / 1e8, paraId, beneficiary);
    }
}